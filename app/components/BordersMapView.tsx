...
/**
 * Renders a target country and its neighboring borders using Leaflet + TopoJSON.
 */
export default function BordersMapView({
  targetCode,
  neighborCodes,
  height = 200,
}: BordersMapViewProps) {
  const targetId = A2_TO_NUM[targetCode] || '';
  const neighborIds = neighborCodes.map(code => A2_TO_NUM[code] || '').filter(Boolean);
  
  const html = useMemo(() => buildShapeHtml(targetId, neighborIds), [targetId, neighborIds]);
...
function buildShapeHtml(targetId: string, neighborIds: string[]): string {
  const neighborsStr = JSON.stringify(neighborIds);
  ...
  <script>
    ...
    fetch('https://cdn.jsdelivr.net/npm/world-atlas@2.0.2/countries-50m.json')
      ...
      .then(function(topology){
        ...
        var neighborFeatures = [];
        
        var targetNum = parseInt(targetId, 10);
        var neighborNums = neighborIds.map(function(id) { return parseInt(id, 10); });

        for(var i=0; i<geojson.features.length; i++){
          var featureId = parseInt(String(geojson.features[i].id), 10);
          if (featureId === targetNum) {
            targetFeature = geojson.features[i];
          } else if (neighborNums.includes(featureId)) {
            neighborFeatures.push(geojson.features[i]);
          }
        }

        if(!targetFeature){
          document.getElementById('loading').style.display='block';
          document.getElementById('loading').textContent='Target missing';
          return;
        }

        // Feature group to calculate bounds for both target and neighbors
        var allLayers = L.featureGroup().addTo(map);

        // Draw neighbors first (below target)
        neighborFeatures.forEach(function(feature) {
          L.geoJSON(feature, {
            style: {
              fillColor: '#5a5a7a',
              fillOpacity: 0.8,
        ...
        // Draw target on top
        var targetLayer = L.geoJSON(targetFeature, {
          style: {
        ...
        // Fit to target bounds instead of taking on the sheer size of massive neighbors like Russia
        // We call invalidateSize() to ensure Leaflet reads the actual WebView dimensions before fitting
        map.invalidateSize();
        map.fitBounds(targetLayer.getBounds().pad(0.15), {animate: false});
      })
      .catch(function(){
