-- Fix: "Most Questions Answered" daily challenge now counts correct answers (score),
-- not total questions attempted. Rename column for clarity; metric key stays 'questions'.
...
AS $$
  ...
  quiz_rows AS (
    SELECT
      ...
      GREATEST(COALESCE(qr.gold_earned, 0), 0) AS gold,
      COALESCE(
  ...
  totals AS (
    SELECT
      ...
      SUM(qr.score)::integer                                            AS questions_answered,
